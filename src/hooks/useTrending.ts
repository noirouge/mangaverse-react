...
export default function useTrending() {
  ...
  const [trendingManga, setTrendingManga] = useState<TTrendingManga>(
    trendingMangaNotFound
  );
  ...
  useEffect(() => {
    setIsLoading(true);
    const currentTrendingManga = trendingMangas.find(
      (manga) => manga.position === position
    );
    if (currentTrendingManga) setTrendingManga(currentTrendingManga);
    else setTrendingManga(trendingMangaNotFound);

     setIsLoading(false);
    const timeoutId = setTimeout(() => {
        setPosition(position < 5 ? position + 1 : 1);
    }, 5000);
